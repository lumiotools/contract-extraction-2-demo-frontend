...
class FedexContractDataExtractionService:
    _lock = Lock()
    ...
    @classmethod
    def extract_earned_discount_table(cls, uploadedFile: File):
        ...
        response = cls.rate_limited_call(chat.send_message, """
            Locate the Earned Discount Table (s) or any other table which follows a similar format in the attached contract.
            Go through all the 3-4 pages of the Earned Discount Table (s) or any other page containing similar format in the contract file.
            Read all the rows on all the pages of Earned Discount Table.
            Extract the Service names listed in the Earned Discount and return them in a structured format.
            ...
            {
                "services": [
                    "string", ... // All Service names found in the Earned Discount Table
                ],
                "validityPeriod": {
                ...
                "chargeType": "string" // Charge type for the Earned Discount Table (eg. "Annual Transportation Charge")
            }
        """)
        ...
        try:
            ...
            charge_type = extracted_data.get("chargeType", "")
        except:
            print("Failed to extract Service names, validity period, or notes from Earned Discount Table")
            services, validity_period, notes = [], {"startDate": "", "endDate": ""}, []
            
        print("Extracted Earned Discount Table services: ", len(services))
        ...
        for i in range(no_of_calls):
            ...
            response = cls.rate_limited_call(chat.send_message, """
                Find the Earned Discount Table(s) or any other table which follows a similar format from the attached contract.
                Find the following Service(s) in the Earned Discount Table: {service_names}.
                
                For each of the above Service(s), extract the "Grace Discount" values if applicable, else return null.
                For each of the above Service(s), extract the "Annual Transportation Charges" band values.
                For each "Annual Transportation Charges" band, extract the corresponding "Earned Discount" values.
                 ...
                 Use the following output schema:
                 {
                     "table_rows": [
                         {
                         "service": "string",
                         "graceDiscount": "percentage (numeric string, or null)",
                         "bandRange": "string", (range in any of these 2 formats "x - y" or "x + ")
                         "currency": "string" (currency of annual transportation charges band eg. USD),
                         "discount": "percentage (numeric string, or null)",
                         "tags": ["string", ...] // Relevant tags for the service (e.g., "Priority Overnight", "Standard Overnight", "Envelope", "Pak", "2 .....)
                         }
                     ]
                 }
                 The bandRange is "Annual Transportation Charges" and should be strictly in any of these 2 formats "x - y" or "x + " only
                 ...
                 **Note:** Extract the "discount" values accurately as written in the contract file based on service and annual transportation charges band.
            """.replace("{service_names}", ", ".join(services_chunk)))
            
            try:
                table_rows = json.loads(response.text.replace("```json\n", "").replace("\n```", "")).get("table_rows", [])
                print("Extracted Earned Discount Table rows for part ",i,": ",len(table_rows))
            except:
            ...
            for row in table_rows:
                try:
                    if row["bandRange"] and "+" in row["bandRange"].lower():
                        min = row["bandRange"].lower().replace(" +", "").replace(" +", "")
                        max = "infinity"
                    else:
    ...
    def extract(cls, contract: UploadFile):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            extracted_earned_discounts_tables = executor.submit(cls.extract_earned_discount_table, uploadedFile)
            ...
            extracted_earned_discounts_table = extracted_earned_discounts_tables.result()
        ...
        tables.append(extracted_earned_discounts_table)
